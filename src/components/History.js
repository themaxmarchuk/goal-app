...
function NodesDisplay ({nodes}) {
  ...
  const NodeRects = nodes.map((node, i) => {
    ...
    const tooltipText = node.stepsTaken === 0 ? 'No tasks completed.' : `${node.stepsTaken} tasks completed.`
  ...
  return (
    <Card className='history'>
      <div className='history__days'>
        <Typography sx={{ top: '1.5rem' }} data-tag='active' className='history__heading'>Mon</Typography>
        <Typography sx={{ top: '2.95rem' }} className='history__heading'>Wed</Typography>
        <Typography sx={{ top: '4.35rem' }} className='history__heading'>Fri</Typography>
      </div>
